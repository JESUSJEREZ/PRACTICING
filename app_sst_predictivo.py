...
#
# COLAB (ejecutar en celda antes de la app):
#   !pip install streamlit scikit-learn pandas plotly openpyxl pyngrok -q
...
# Mapa columnas originales (con tildes) -> nombre interno sin tildes
COL_RENAME_ORIG = {
...
def prepare_features(file_bytes):
    ...
    mdf["antiguedad"] = pd.to_numeric(
        mdf["antiguedad"].astype(str).replace("Sin informacion", np.nan),
        errors="coerce",
    ).fillna(5)
    ...
    for col in cat_cols:
        le = LabelEncoder()
        mdf[col] = mdf[col].fillna("Sin informacion").astype(str)
...
def train_model(file_bytes, target, algo):
...
with tab1:
    ...
    tmp = raw_df[["Puesto/Cargo", "Total dias de incapacidad"]].copy()
    tmp["Total dias de incapacidad"] = pd.to_numeric(
        tmp["Total dias de incapacidad"], errors="coerce"
    )
    avg_cargo = (
        tmp.groupby("Puesto/Cargo")["Total dias de incapacidad"]
        .mean()
